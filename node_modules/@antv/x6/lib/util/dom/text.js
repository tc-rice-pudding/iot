...
function text(elem, content, options) {
    ...
    var strFontSize = attr_1.attr(elem, 'font-size');
...
function splitText(text, separator, eol, hyphen) {
    ...
    for (var i = 0; i < words.length; i += 1) {
        ...
        if (word.indexOf(eol) >= 0 && word.length > 1) {
            ...
            words.splice.apply(words, __spreadArray([i, 1], valids));
            var seps = valids.map(function () { return ''; });
            seps.pop();
            separators.splice.apply(separators, __spreadArray([i, 0], seps));
        }
    }
    // hyphen
    for (var i = 0; i < words.length; i += 1) {
...
function breakText(text, size, styles, options) {
    ...
    var telem = elem_1.createSvgElement('text');
    var tspan = elem_1.createSvgElement('tspan');
    var tnode = document.createTextNode('');
    attr_1.attr(telem, styles);
