...
var StatusField = /** @class */ (function (_super) {
    ...
    StatusField.prototype.render = function () {
        ...
        if (!status.icon && !status.label) {
            return (React__default["default"].createElement("span", { className: cx('StatusField', className), style: style },
                React__default["default"].createElement("span", { className: "text-muted", key: "status-value" }, placeholder)));
        }
        var wrapClassName = '';
        // icon element
        ...
        if (status.icon) {
            ...
            if (svgIcon_1) {
                iconElement = (React__default["default"].createElement(amisUi.Icon, { icon: svgIcon_1, className: cx('Status-icon icon', itemClassName), key: "icon" }));
            }
            else {
                iconElement = amisCore.generateIcon(cx, icon, 'Status-icon');
            }
        }
        var labelElement = null;
        if (status.label !== '' && status.label != null) {
            labelElement = (React__default["default"].createElement("span", { className: cx('StatusField-label'), key: "label" }, amisCore.filter('' + status.label, data)));
        }
        return (React__default["default"].createElement("span", { className: cx('StatusField', wrapClassName, className, status.className), style: tslib.__assign(tslib.__assign({}, style), (status.color ? { color: amisCore.filter(status.color, data) } : {})) },
            iconElement,
            labelElement));
    };
