            ...
            return __generator(this, function (_b) {
                switch (_b.label) {
                    case 0:
                        ...
                        selectedItems = value
                            ? __spreadArray(__spreadArray([], __read(store.selectedRows.map(function (row) { return row.data; })), false), [item.data], false) : store.selectedRows.filter(function (row) { return row.id !== item.id; });
                        unSelectedItems = value
                            ? store.unSelectedRows.filter(function (row) { return row.id !== item.id; })
                            : __spreadArray(__spreadArray([], __read(store.unSelectedRows.map(function (row) { return row.data; })), false), [item.data], false);
        ...
        if (scrollLeft !== this.lastScrollLeft) {
            ...
            if (fixedLeft && fixedLeft.length) {
                for (var i = 0, len = fixedLeft.length; i < len; i++) {
                    ...
                    // 同步thead高度
                    forEach(node.querySelectorAll('thead>tr>th'), function (item) {
                        if (theadHeight) {
        ...
        return (React.createElement("table", { className: cx('Table-table', tableClassName, {
            ...
            headerOnly ? null : !rows.length ? (React.createElement("tbody", null,
                React.createElement("tr", { className: cx('Table-placeholder') },
                    React.createElement("td", { colSpan: columns.length })))) : (React.createElement(TableBody, { tableClassName: cx(tableClassName, {
                    'Table-table--withCombine': store.combineNum > 0
