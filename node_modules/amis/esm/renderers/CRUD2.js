...
import { parseQuery, getPropValue, isArrayChildrenModified, isObjectShallowModified, isApiOutdated, isPureVariable, resolveVariableAndFilter, qsparse, qsstringify, createObject, isEffectiveApi, evalExpression, filter, extendObject, getVariable, autobind, ScopedContext, Renderer, CRUDStore } from 'amis-core';
...
var CRUD2 = /** @class */ (function (_super) {
    ...
    CRUD2.prototype.handleSearch = function (data) {
        ...
        var _b = data || {}, query = _b.query, resetQuery = _b.resetQuery, replaceQuery = _b.replaceQuery, loadMore = _b.loadMore;
        query =
            syncLocation && query
    ...
    CRUD2.prototype.handleSave = function (rows, diff, indexes, unModifiedItems, rowsOrigin, options) {
        ...
        if (Array.isArray(rows)) {
            ...
            store
                ...
                .then(function () {
                reload && _this.reloadTarget(filter(reload, data_1), data_1);
            ...
            store
                ...
                .then(function () {
                reload && _this.reloadTarget(filter(reload, data_2), data_2);
                _this.getData(undefined, undefined, true, true);
            })
                .catch(function () {
    ...
    CRUD2.prototype.handleSaveOrder = function (moved, rows) {
        ...
        isEffectiveApi(saveOrderApi, model) &&
            store
                ...
                .then(function () {
                reload && _this.reloadTarget(filter(reload, model), model);
                _this.getData(undefined, undefined, true, true);
            })
                .catch(function () { });
    };
    ...
    CRUD2.prototype.renderChild = function (region, schema, props) {
        ...
        return render(region, schema, __assign(__assign({ 
            // 包两层，主要是为了处理以下 case
            // 里面放了个 form，form 提交过来的时候不希望把 items 这些发送过来。
            // 因为会把数据呈现在地址栏上。
            /** data 可以被覆盖，因为 filter 中不需要额外的 data */
            data: createObject(createObject(store.filterData, store.getData(this.props.data)), {}) }, props), childProps));
    };
